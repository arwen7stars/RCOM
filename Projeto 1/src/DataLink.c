...
int stateMachine(ConnectionState * state, char ch){
	...
	switch(*state) {
		...
		case A_RCV:
			if(ch == C_UA || ch == C_SET || ch == C_DISC || ch == C_RR0 || ch == C_RR1 || ch == C_REJ0 ||
ch == C_REJ1){
				*state = C_RCV;
				control_field = ch;
			}
...
int dataStateMachine(ConnectionState * state, char ch){
...
int llread(int fd, unsigned char* buffer){
	...
	while(!received){
		if(receivePacket(fd, packet, &packetSize)){
			...
			if(!verifyDataReceived(packet, size)){
...
int llclose (int fd, int type){
	...
	if(type == TRANSMITTER){
		...
		int try = 0;
   		...
   		while(try < NUM_TRIES) {
    		...
    		if(write(fd,DISC,sizeof(DISC)) < 0){		// writes frame DISC and checks if it was successful
    			...
    			return -1;
    		} else printf("Disc sent successfully\n");
		...
		} else printf("Error receiving UA.\n");
